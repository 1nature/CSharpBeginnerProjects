namespace QuizMaker
{
    public static class UIMethod
    {
        ...
        {
            Console.WriteLine("*********************************************");
            Console.WriteLine($"Enter {Constant.QUIZ_DECISION_YES} to restart the quizmaker, or {Constant.QUIZ_DECISION_NO} to quit.");
            int newQuestion;
            do
            {
        ...
        public static bool StopStoringQuestion(char userAnswer, List<QuestionandAnswer> questionStorage, bool restartStoring, int totalQuestion)
        {
            if (userAnswer == Constant.QUIZ_TYPE_STOREONLY)
            {
                ...
                {
                    ShowCompletedQuestionMessage();
                    restartStoring = RestartQuiz();
                    if (!restartStoring)
                    {
                        DisplayQuitMessage();
        ...
        public static bool ShowScoreForRestartOrQuit(int exitVariable, int currentScore, int maxScore)
        {
            ...
            if (exitVariable < Constant.MINIMUM_NUMBER_OF_QUESTION)
            {
                ...
                if (restartVariable)
                {
                    ...
                    decimal theResult = (decimal)currentScore / maxScore * Constant.MAX_SCORE;
        ...
        {
            Console.WriteLine($"Enter {Constant.QUIZ_DECISION_YES} to restart the quiz, or {Constant.QUIZ_DECISION_NO} to quit.\n");
            int restart;
            do
            {
        ...
        {
            Console.WriteLine($"Enter {Constant.QUIZ_DECISION_YES} to store and/or answer question(s), or {Constant.QUIZ_DECISION_NO} to quit. \n");
        }
        ...
        public static void DisplayQuizLogic(bool playOn, char quizChoice)
        {
            ...
            while (playOn) 
            {
                ...
                {
                    storedQuestions = GetUserInputandCreateNewQuestionsandAnswers(ListOfQuestions);
                    playOn = StopStoringQuestion(quizChoice, ListOfQuestions, playOn, storedQuestions);
                ...
                if (quizChoice == Constant.QUIZ_TYPE_STOREANDANSWER || quizChoice == Constant.QUIZ_TYPE_ANSWERONLY)
                {
                    ...
                    DisplayUserInstruction(numOfAvailableQuestions);
